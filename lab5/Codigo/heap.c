...
int FixDown(Heap * h, int k)
{
  ...
  while ((2 * k + 1) < h->n_elements) { 
    ...
    n_ops++;
    if (((j + 1) < h->n_elements) &&
        (h->less) (h->heapdata[j], h->heapdata[j + 1])) {   /* operation */
      /* second offspring is greater */
      j++;
    }
    n_ops++;
    if (!(h->less) (h->heapdata[k], h->heapdata[j])) {   /* operation */
...
void HeapSort(Heap * h)
{
    ...
    n_ops += Heapify(h);  /* operation */
    auxN = h->n_elements;

    for(i = 0; i < auxN; i++){  /* operation */
        t = h->heapdata[0]; /* operation */
        h->heapdata[0] = h->heapdata[h->n_elements - 1]; /* operation */
        h->heapdata[h->n_elements - 1] = t;  /* operation */
        h->n_elements--;
        n_ops += 4;
    ...
    fprintf(stdout, "[HEAP SORT] total number of ops: %d\n", n_ops);
